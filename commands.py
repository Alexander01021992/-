...
async def start(message: Message, state: FSMContext) -> None:
    ...
    if is_old_user_flag and not is_paying_user:
        ...
        welcome_text += (
            ...
            escape_md("✔️ Или выбери только аватар за 690₽\n", version=2) +
...
async def help_command(message: Message, state: FSMContext) -> None:
    ...
    help_text = (
        ...
        escape_md("   - 🎥 AI-видео (Kling 2.1): Преврати статичное фото в короткое динамичное видео. Требуется 30 печенек с баланса.", version=2) + "\n\n" +
...
async def check_training(message: Message, state: FSMContext, user_id: int) -> None:
    """Проверяет статус обучения аватаров."""
    user_id = message.from_user.id
    if await check_user_blocked(message):
        return

    bot = message.bot
    logger.info(f"Проверка статуса обучения для user_id={user_id}")
    await reset_generation_context(state, "check_training_logic")

    try:
        trained_models = await get_user_trainedmodels(user_id)
        if not trained_models:
            text = (
                escape_md("🚫 У тебя пока нет аватаров в обучении или готовых. Хочешь создать новый? 🛠", version=2) + "\n" +
            ...
            logger.debug(f"Сообщение об отсутствии аватаров отправлено для user_id={user_id}: {text}")
            return

        response_text = escape_md("📋 Статус твоих аватаров:", version=2) + "\n\n"
                ...
                if status in ['pending', 'starting', 'processing']:
                    ...
                    if not model_name_for_check and trigger_word_db_check:
                        ...
                        if REPLICATE_USERNAME_OR_ORG_NAME:
                            model_slug_reconstructed = f"flux-{user_id}-{trigger_word_db_check.lower().replace('_', '-')}"
        ...
        logger.debug(f"Статус обучения отправлен для user_id={user_id}: {response_text}")
        
    except Exception as e:
        logger.error(f"Ошибка в check_training для user_id={user_id}: {e}", exc_info=True)
        error_text = escape_md("❌ Ошибка при проверке статуса обучения.", version=2)
        main_menu_kb = await create_main_menu_keyboard(user_id)
        await message.answer(
            error_text,
            reply_markup=main_menu_kb,
