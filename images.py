...
# Изменено: Обновлена конфигурация на основе generation_config.py для максимального качества.
# Изменено: Увеличена сила для ключевых LoRA (skin_texture_master до 1.0, anti_cgi до 0.9, ultra_realism до 0.9 и т.д.) для усиления фотореализма.
# Изменено: Добавлены новые LoRA из HF_LORA_MODELS, такие как "strangerzonehf/Flux-Super-Realism-LoRA", "XLabs-AI/flux-RealismLora" и другие для большего разнообразия и качества.
# Изменено: Приоритеты скорректированы для обязательного включения "skin_texture_master", "photo_realism_pro", "anti_cgi" в начале.
ULTRA_PROFESSIONAL_LORA_CONFIG = {
    ...
    "ultra_realism": {
        "model": "prithivMLmods/Flux-Ultra-Realism",
        "strength": 0.9,
        "keywords": ["ultra realistic", "hyperrealistic", "extreme detail"],
    ...
    "super_realism": {
        ...
        "strength": 0.75,
        "keywords": ["super realistic", "hyperrealistic", "extreme detail"],
    ...
    "realism_xl": {
        ...
        "strength": 0.85,
        "keywords": ["realistic", "natural", "authentic"],
...
# Изменено: Обновлены пресеты на основе generation_config.py для ультра-качества.
# Изменено: Установлено num_inference_steps = 50 (максимум, как указано в запросе - больше нельзя).
# Изменено: Установлено guidance_scale = 4.0 для всех пресетов, чтобы соответствовать GENERATION_QUALITY_PARAMS["ultra_max_quality"].
# Изменено: Добавлены дополнительные LoRA из нового config, такие как "ultra_realism", "better_skin" во все пресеты.
# Изменено: Усилены prompt_additions для борьбы с пластиковым блеском и повышения сходства: добавлены "imperfect skin, natural variations in skin tone, no shine, matte skin finish".
ULTRA_PHOTOREALISTIC_PRESETS = {
    "natural_portrait": {
        "loras": ["skin_texture_master", "photo_realism_pro", "anti_cgi", "face_perfection_v2", "better_skin", "ultra_realism"],
        "guidance_scale": 4.0,
    ...
    "studio_perfection": {
        "loras": ["photo_realism_pro", "color_grading_pro", "face_perfection_v2", "ultra_details", "better_skin", "anti_cgi", "ultra_realism"],
        "guidance_scale": 4.0,
        "num_inference_steps": 50,
        "prompt_additions": (
    ...
    "lifestyle_natural": {
        "loras": ["skin_texture_master", "anti_cgi", "photo_realism_pro", "portrait_master_pro", "better_skin", "ultra_realism"],
        "guidance_scale": 4.0,
        "num_inference_steps": 50,
        "prompt_additions": (
...
# Изменено: Обновлен на основе NEGATIVE_PROMPTS["ultra_realistic_max"] из generation_config.py для максимального качества.
# Изменено: Добавлены дополнительные элементы против пластикового блеска и артефактов: "plastic shine, glossy skin, reflective skin, shiny pores, oily reflectance, cgi skin, rendered skin, artificial reflectance, fake shine, perfect symmetry, unnatural perfection, over-smoothed features".
        ...
        if generation_type in ['with_avatar', 'photo_to_photo']:
            ...
            if trigger_word and old_model_version:
                if old_model_id:
                    ...
                    params[f"lora_scale_{lora_index}"] = USER_AVATAR_LORA_STRENGTH  # Используем максимум 1.0 из config
                    lora_index += 1
                    logger.info(f"Добавлен пользовательский LoRA с силой {USER_AVATAR_LORA_STRENGTH}")
