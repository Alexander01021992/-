...
async def init_db(bot: Bot = None) -> None:
    """Инициализирует базу данных, создавая все необходимые таблицы с индексами."""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            # Новая таблица для кнопок рассылки
            ...
            indices = [
                ...
                ('idx_broadcast_buttons_broadcast', 'broadcast_buttons(broadcast_id)')  # Новый индекс
            ]
            ...
            await conn.commit()
            logger.info("База данных успешно инициализирована с индексами и триггерами")
            ...
            if bot:
                ...
                if referral_photos > 0 and referrer_id:
                    try:
                        ...
                        if referrer_data:
                            message_text = safe_escape_markdown(
                                f"🎁 Ваш друг оплатил подписку! Вам начислено {referral_photos} печеньки за реферала!\n"
...
async def get_payments_by_date(start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[Tuple]:
    """Получает платежи за указанный период"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()
            ...
            return [
                (
                    ...
                    p['amount'],
                    p['payment_id'],
                    datetime.strptime(p['created_at'], '%Y-%m-%d %H:%M:%S') if p['created_at'] else None,
                    p['username'],
                    p['first_name']
                )
                for p in payments
